...
export default function DashboardSidebar({
  ...
  // ── Shared nav list (used by both desktop & mobile) ──
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 bg-background border-r flex flex-col transition-transform duration-200 ease-in-out md:hidden",
      ...
      <aside
        className={cn(
          "hidden md:flex md:flex-col md:fixed md:inset-y-0 bg-background border-r z-40 transition-[width] duration-200 ease-in-out",
      ...
      >
        ...
        <div
          className={cn(
            "flex items-center h-14 border-b transition-all duration-200",
            isCollapsed ? "justify-center px-2" : "px-4"
          )}
        >
          <Link
            href="/dashboard"
            className={cn("flex items-center gap-2.5", isCollapsed && "gap-0")}
          >
            <div className="w-8 h-8 rounded-md bg-primary flex items-center justify-center shrink-0">
              <FileText className="h-4 w-4 text-primary-foreground" />
            </div>
            {!isCollapsed && (
              <span className="font-semibold text-sm whitespace-nowrap">Blog Admin</span>
            )}
          </Link>
        </div>
